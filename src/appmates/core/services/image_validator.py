...
from appmates.core.specs.registry import SizeSpec, StoreSpec, load_spec
...
class ScreenshotValidator:
    ...
    def _check_format(self, facts: ImageFacts, spec: StoreSpec, store: Store) -> Iterable[Finding]:
        ...
        if facts.has_alpha and not rules.allow_alpha:
            yield Finding(
                ...
                fix_hint="Flatten onto an opaque background: `appmates fix-screenshots`.",
                fixable=True,
            )
        ...
        if rules.required_color_spaces and facts.mode not in _RGB_MODES:
            yield Finding(
                ...
                fix_hint="Convert to sRGB: `appmates fix-screenshots`.",
                fixable=True,
            )
    ...
    def _check_apple_size(
        ...
        if hit is None:
            ...
            hint = (
                f"Closest accepted size is {nearest.label}." if nearest else "See `appmates specs`."
            )
            findings.append(
                Finding(
        ...
        if hit.is_deprecated:
            findings.append(
                Finding(
                    ...
                    fix_hint="Re-export at a current size; see `appmates specs --store apple`.",
                    fixable=True,
                )
            )
        elif hit.is_legacy:
