...
from appmates.core.models.testing import ClosedTestingStatus
from appmates.core.services.asc_sync import PulledListing, PushPlan
from appmates.core.services.image_fixer import FixResult
from appmates.core.services.metadata_validator import MetadataReport
from appmates.core.services.pricing_calculator import PricingPlan
from appmates.core.services.submission_checker import SubmissionReadinessReport
from appmates.core.specs.registry import StoreSpec
...
def render_asc_pull(pulled: PulledListing) -> RenderableType:
    title = "Pulled from App Store Connect" + (
        " · live, published version" if pulled.is_live else ""
    )
    table = Table(
        title=title,
        title_style="bold",
    ...
    for meta in pulled.listing.locales:
        table.add_row(meta.locale, meta.title or "—", meta.subtitle or "—", meta.keywords or "—")

    parts: list[RenderableType] = [table]
    if pulled.is_live:
        parts.append(
            Text(
                "This app has no draft version, so this is what's live right now — read-only in "
                "practice: pushing edits needs a new version created in App Store Connect first.",
                style="dim italic",
            )
        )
    if pulled.current_prices:
        price_table = Table(title="Current prices", title_style="bold", header_style="bold")
        price_table.add_column("Territory")
        price_table.add_column("Price", justify="right")
        for row in pulled.current_prices:
            price_table.add_row(str(row.get("territory")), str(row.get("price")))
        parts.append(price_table)
    else:
        parts.append(Text("No price schedule found.", style="dim italic"))
    return Group(*parts)


def render_asc_push_plan(plan: PushPlan) -> RenderableType:
    table = Table(
        title="Would change",
        title_style="bold",
        header_style="bold",
    ...
    for change in plan.changes:
        table.add_row(
            change.locale,
            change.field,
            Text(change.old or "(empty)", style="dim"),
            Text(change.new or "(empty)", style="bold green"),
        )
    return table
