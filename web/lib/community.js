  ...
  async _request(path, { method = 'GET', body, credentials = 'include' } = {}) {
    ...
    try {
      response = await this.fetchImpl(`${this.baseUrl}${path}`, {
        method,
        credentials,
        // Every response here is recomputed per request server-side (see
        // the matching `cache-control: no-store` the Worker sends back) —
        // a browser-cached GET is exactly how "the leaderboard/listings
        // don't update" bugs happen, so this isn't left to heuristics on
        // either end.
        cache: 'no-store',
